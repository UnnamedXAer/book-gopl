...
func makeThumbnails5(filenames <-chan string) int64 {
	log.Println("makeThumbnails5 - start")
	...
	for f := range filenames {
		...
		go func(f string) {
			...
			log.Println("thumbfile", thumbfile)

			sizes <- info.Size()
			log.Println("thumbfile - sizes", info.Size())

		}(f)

		log.Println("f", f)
	}
	log.Println("afterloop")
	...
	log.Println("about to total")
	var total int64
	for size := range sizes {
		log.Println("sizes", size)
		total += size
	}
	return total
}
